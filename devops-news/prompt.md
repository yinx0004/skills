使用skill-creator 创建一个 devops-news 资讯skill，用于获取每日最新资讯

## Step 1：数据获取
使用agent-browser skill分别访问以下3个数据源：
1. CNCF Blog￼- `https://www.cncf.io/news/`
2. InfoQ￼- `https://www.infoq.com/continuous_delivery/`
3. Cloudflare Blog￼- `https://blog.cloudflare.com/tag/speed-and-reliability/`

将抓取到的数据整理为以下格式，保存为 `devops-news-data.js`：
```js
const NEWS_DATA = [
  {
    "title": "文章标题",
...
2. `devops-news.html` — 展示页面（通过 script 标签引入数据）
